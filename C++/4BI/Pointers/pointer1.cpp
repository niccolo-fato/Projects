...
e viene messo in coda, quando il medico è libero viene estratto il primo paziente 
della coda e viene visitato.
Si realizzi un programma che preveda tre opzioni:
1.arrivo di un paziente;
2.estrazione del paziente dalla coda;
...
public:
    patients(){ //costruttore
        head = NULL;
        end = NULL; //inizializzazione della coda e testa a vuoto
        num = 1;
    }
    void push(){ //estrazione del paziente in coda
        patient *p;
        p = new (patient);
    ...
    void pop(){ //estrazione del primo paziente in coda
        patient *p;
        if (head == NULL)
            ...
            p = head;
            head = p->next;
            cout << "Il prossimo da visitare e' " << p->name << " e ha il  numero " << p->num << "\n";
            if (head == NULL)
                end = NULL;
            delete p;
        }
    }
    void print(){ //stampa dei pazienti in attesa
        patient *p;
